...
export default function App() {
  ...
  const refreshRecords = useCallback(async () => {
    ...
    try {
      ...
      const data = await response.json() as { records?: HubRecord[] };
      if (!Array.isArray(data.records)) throw new Error("Resposta inválida da API de coletas.");
      setRecords(data.records);
      setLastSyncAt(new Date());
    } catch (error) {
      console.warn("API de coletas indisponível; mantendo os registros atuais.", error);
    } finally {
  ...
  useEffect(() => {
    if (view !== "hub") return;
    void refreshRecords();
    const intervalId = window.setInterval(() => void refreshRecords(), 10000);
  ...
  return <div className="flex h-screen w-screen flex-col overflow-hidden bg-[#f4f7fb] font-sans text-slate-800">
    ...
    <div className="flex min-h-0 flex-1">{view === "hub" ? <HubDashboard records={records} isRefreshing={isRefreshing} lastSyncAt={lastSyncAt} onRefresh={() => void refreshRecords()} onRecordsImported={(newRecords) => { setRecords((current) => [...newRecords, ...current]); const canonicalPayloads = newRecords.filter((record) => record.payload.schemaVersion === "1.0"); void Promise.all(canonicalPayloads.map((record) => fetch("/api/collections", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(record.payload) }))).catch((error) => console.warn("Não foi possível sincronizar a importação com a API.", error)); }} onOpenChat={() => setView("chat")} /> : <ChatInterface messages={messages} onSendMessage={handleSendMessage} isLoading={isLoading} onOpenWizard={() => setIsWizardOpen(true)} onSelectPreset={(preset: HardwarePreset) => void handleSendMessage(preset.prompt)} />}</div>
